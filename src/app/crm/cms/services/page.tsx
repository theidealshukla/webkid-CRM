...
export default function ServicesPage() {
  ...
  async function patch(id: string, p: Partial<Service>) {
    setRows((r) => r.map((x) => (x.id === id ? { ...x, ...p } : x)));
    try { await api.update(id, p); await revalidatePublic("cms:services"); }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {rows.map((r) => (
          <div key={r.id} className="border border-gray-200 rounded-lg p-4 space-y-3 bg-white">
            ...
            <input value={r.tagline ?? ""} placeholder="Tagline" onChange={(e) => patch(r.id, { tagline: e.target.value })} className="input text-sm"/>
            <textarea rows={2} value={r.description ?? ""} placeholder="Description" onChange={(e) => patch(r.id, { description: e.target.value })} className="input text-sm"/>
            <input value={r.icon_name ?? ""} placeholder="Lucide icon name (e.g. Code, Palette)" onChange={(e) => patch(r.id, { icon_name: e.target.value })} className="input text-xs font-mono"/>
